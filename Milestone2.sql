...
@acc int output
As
if(exists(select * from Course where id=@courseId and accepted=1))
set @acc =1
else
UPDATE Course
...
else if(exists(select * from Course where id=@cid and instructorId=@instrId))
set @success =2
else if(exists(select * from StudentTakeAssignment S inner join Course C 
on C.id =S.cid where cid=@cid and assignmentNumber=@assignmentNumber and sid=@sid  and assignmenttype=@type))
set @success =3
else set @success=0
go
...
exists(select * from StudentTakeCourse where sid=@sid and grade is null and cid=@cid))
set @success=2
else if(
exists(select * from StudentTakeCourse where sid=@sid and grade <= 2.0 and cid=@cid))
set @success=3
...
begin
print 'This course is not accepted'
set @success=4
end
end
else
...
@success int output
AS
IF(EXISTS(SELECT * FROM StudentTakeAssignment STC 
WHERE STC.cid = @cid AND STC.sid = @sid AND STC.assignmentNumber= @assignnumber AND STC.assignmenttype = @assignType ))
begin
...
insert into Users values('nour', 'ahmed','123', 1, 'n@g.com', 'tagamo')
insert into Admin(id) values(3)
insert into Course values (8,'DB501', 'course', 100, 'link', 3,1,1)
insert into Course values (8,'CSEN401', 'course', 100, 'link', 3,1,0)
insert into Promocode values('123','7/7/2020', '2/2/2021', 20, 3)
insert into StudentHasPromocode values(2,'123')
insert into InstructorTeachCourse values(1,2)
insert into InstructorTeachCourse values(1,1)
...
insert into assignment values(1, 1, 'project', 100, 20, '2/2/2021', 'assignment')
insert into assignment values(1, 2, 'quiz', 100, 20, '2/2/2021', 'assignment')
insert into StudentTakeAssignment values(2,1,2,'quiz',100)


insert into Course values (8,'Media501', 'course', 100, 'link', 3,1,1)

insert into assignment values(3, 1, 'quiz', 100, 100, '8/8/2020', 'assignment')
insert into StudentTakeAssignment values(2,3,1,'quiz',100)

insert into StudentCertifyCourse values(2,3, '10/10/2020')
insert into StudentTakeCourse values(2,3,1,1,90)
insert into InstructorTeachCourse values(1,3)
